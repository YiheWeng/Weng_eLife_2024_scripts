...
% This script used to generalize the FU3 model to external dataset
train_group = 'FU3';
test_group = 'Stratify';
...
% train mats,covars,behav for FU3 (age 23)
...
train_covars = readtable(fullfile('W:\Yihe\Results_Shen_GT\ICV',train_group,'cpm_mat\site_covariates.csv'));
train_covars = train_covars{:,2:end};

% testing mats,covars,behav for STRATIFY data
...
test_covars = readtable(fullfile('W:\Yihe\Both_Results_Shen\ICV',test_group,'cpm_mat\covariates.csv')); % stratify data
...
% apply the models identified from FU3 (train data) to the external dataset
% (test data)
id_pd = []; 
